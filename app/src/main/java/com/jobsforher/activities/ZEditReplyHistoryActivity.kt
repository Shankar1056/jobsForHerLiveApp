...
import kotlinx.android.synthetic.main.layout_edithistory_replies.*
...
class ZEditReplyHistoryActivity : BaseActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.layout_edithistory_replies)
        id=intent.getIntExtra("id",0)
        icon=intent.getStringExtra("icon")
        Log.d("TAGG2", "INSIDE functioj"+icon)
        editRepliesHistoryData(id)
//        if(icon.isNotEmpty()) {
...
//        }
    ...
    fun editRepliesHistoryData(id: Int){
...
//        Toast.makeText(applicationContext, id.toString()+"", Toast.LENGTH_LONG).show()
        val call = retrofitInterfaceposts!!.getEditedRepliesHistory(id, EndPoints.CLIENT_ID, "Bearer "+EndPoints.ACCESS_TOKEN)
