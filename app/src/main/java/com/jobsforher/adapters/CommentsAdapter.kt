...
class CommentsAdapter(private val mDataList: ArrayList<GroupsCommentModel>) : RecyclerView.Adapter<CommentsAdapter.MyViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MyViewHolder {
        var view = LayoutInflater.from(parent.context).inflate(R.layout.comments_adapter_row, parent, false)
    ...
    override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
        ...
        holder.comment_datetime.text = mDataList[position].created_on_str
        holder.comment_data.text= Html.fromHtml(mDataList[position].entity_value)
        holder.comment_upvote.text= mDataList[position].upvote_count
        holder.comment_downvote.text= mDataList[position].downvote_count
        holder.comment_replies.text= mDataList[position].replies_counts
        ...
        Log.d("TAGG", "REPLY COUNT"+mDataList[position].replies_counts)
        holder.view_all_replies.setOnClickListener {
            val intent = Intent(context, ZActivityRepliesPage::class.java)
            intent.putExtra("comment_Id", mDataList[position].id)
            intent.putExtra("comment_name", mDataList[position].username)
            intent.putExtra("comment_datetime", mDataList[position].created_on_str)
            intent.putExtra("comment_desc", mDataList[position].entity_value)
            intent.putExtra("comment_icon", mDataList[position]!!.profile_icon)
        ...
        if (mDataList[position].replies_counts.toString().equals("0")){
            holder.replyblock.visibility = View.GONE
        }else{

            holder.replyblock.visibility = View.VISIBLE
//            Log.d("TAGG","ADAPTER coment"+mDataList[position].comment_list!!.entity_value)
            holder.reply_name.setText(mDataList[position].reply_list!!.username!!)
            holder.reply_datetime.setText(mDataList[position].reply_list!!.created_on_str)
            holder.reply_data.setText(Html.fromHtml(mDataList[position].reply_list!!.entity_value))
            ...
            if(mDataList[position].reply_list!!.profile_icon!!.isNotEmpty()) {
                Picasso.with(context)
                    .load(mDataList[position].reply_list!!!!.profile_icon)
                    .placeholder(R.drawable.ic_launcher_foreground)
...
//        }
        ...
        holder.longclick_block.setOnClickListener {
...
//        holder.reply_block_whole.setOnLongClickListener{
//            editReply(position)
//
//            true
//        }
...
//        }
    ...
    inner class MyViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        internal var reply_block_whole: LinearLayout
        ...
        init {
            ...
            replyblock = itemView.findViewById<View>(R.id.reply_block) as LinearLayout
            ...
            icadd  =itemView.findViewById<View>(R.id.ic_add) as ImageView
            ...
            reply_block_whole = itemView.findViewById<View>(R.id.reply_block_whole) as LinearLayout
...
//                        mDataList[position].reply_list!!.replies_count = jsonaobj.getString("replies_count")
    ...
    private fun checkReportComment(pos:Int){
        ...
        Log.d("TAGG", "ID is "+mDataList[pos].id.toString())
        val call = retrofitInterface!!.ReportCheckComment(mDataList[pos].id,EndPoints.CLIENT_ID, "Bearer "+EndPoints.ACCESS_TOKEN)
        ...
        call.enqueue(object : Callback<DeletePostResponse> {
            override fun onResponse(call: Call<DeletePostResponse>, response: Response<DeletePostResponse>) {
                ...
                if (response.isSuccessful && response.body()!!.responseCode.toString().equals("10804") ) {      //11804
                    // Toast.makeText(context,"Hello",Toast.LENGTH_LONG).show()

                    (context as ZActivityCommentPage).openBottomSheet(mDataList[pos].edited,mDataList[pos].id, mDataList[pos].entity_value, mDataList[pos].is_owner,
        ...
        call.enqueue(object : Callback<DeletePostResponse> {
            override fun onResponse(call: Call<DeletePostResponse>, response: Response<DeletePostResponse>) {
                ...
                if (response.isSuccessful && response.body()!!.responseCode.toString().equals("11804") ) {      //11804
                    // Toast.makeText(context,"Hello",Toast.LENGTH_LONG).show()
                    (context as ZActivityCommentPage).openBottomSheetReplies(mDataList[pos].reply_list!!.id, mDataList[pos].reply_list!!.entity_value, mDataList[pos].reply_list!!.is_owner,
                        mDataList[pos].profile_icon!!.toString(),"no")
                } else {
